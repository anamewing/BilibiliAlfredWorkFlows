...
def WriteCommentsXmlFile(input_files, output_file):
    fo = ConvertToFile(output_file,'w')
    fo.write(input_files)
    fo.close()
...
if cid != '':
    t = cid.split('------')
    t[1] = t[1].replace(r'/','');
    WriteCommentsXmlFile(GetDanmuku(t[0]),r'%s/Desktop/%s-%s.xml'%(os.path.expanduser('~'),t[1],t[2]))
